...
public class GameSelect : MonoBehaviour
{
    ...
    //{
    //    var gameInfo = JsonUtility.FromJson<GameInformation>(json);
    ...
    public void CreateGame()
    {
        ...
        var newGameInfo = new GameInformation();

        newGameInfo.size = sizeSlider.GetComponent<ValueToText>().GetValue("SizeSlider");
        newGameInfo.colors = colorSlider.GetComponent<ValueToText>().GetValue("ColorAmountSlider");

        newGameInfo.seed = Random.Range(0, int.MaxValue);
        newGameInfo.displayName = PlayerData.data.myName + "'s game";

        //Add the user as the first player
        newGameInfo.players = new List<UserInformation>();
    ...
    public void GameCreated(string gameKey, GameInformation gameInfo)
    {
        ...
        //Start the game
        SceneLoader.SceneInstance.StartGame(gameInfo);
    }
    ...
    //public void JoinGame(GameInformation gameInfo)
    //{
    ...
    //    gameInfo.displayName = gameInfo.players[0].myName + " vs " + PlayerData.data.myName;
